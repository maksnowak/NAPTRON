...
def parse_args_nap():
    parser = argparse.ArgumentParser(description="NAP")
...
def append_uncertainty(bboxes, uncertainty, labels):
    ...
    uncertainty_bbox_fmt = [
        [uncertainty[j][labels[j] == i] for i in range(len(bboxes[0]))]
...
def get_tp_activations(certainties, activations, bboxes, labels, iou=0, score_thr=0.0):
    activations_bbox_fmt = [
        [activations[j][labels[j] == i] for i in range(len(bboxes[0]))]
        for j in range(len(bboxes))
    ]
    mask = certainties["tp_logit_certainty"][iou][0] > score_thr
    train_tp_activations = torch.zeros(
        (len(certainties["tp_img_id"][iou][0][mask]), activations[0].shape[-1]),
        device="cuda",
    )
    train_tp_labels = np.zeros((len(certainties["tp_img_id"][iou][0][mask])))

    for i, img_id in tqdm(
        enumerate(certainties["tp_img_id"][iou][0][mask]),
        total=train_tp_labels.shape[0],
    ):
...
def prepare_nap_monitor(args):
    ...
    train_bboxes, train_activations, shapes, train_labels = parse_nap_detector_outputs(
        train
    )
    ...
    try:
        train_dataset.evaluate_ood(
            train_bboxes, metric=["uncertainty"], dump_certainties=True
        )
    ...
    train_tp_activations, train_tp_labels = get_tp_activations(
        train_certainties,
        train_activations,
        train_bboxes,
        train_labels,
    ...
    train_tp_activations, monitored_layers_shapes = prepare_patterns(
        train_tp_activations, shapes, int(args.layer_id)
    )

    monitor = FullNetMonitor(
        class_num, train_tp_activations.device, layers_shapes=monitored_layers_shapes
    )
...
def binarize_patterns(layers, q=0.0):
...
def parse_nap_detector_outputs(outputs):
    bboxes, activations, shapes, labels = (
        outputs[::4],
        outputs[1::4],
        outputs[2::4],
        outputs[3::4],
...
if __name__ == "__main__":
    ...
    test_bboxes, test_activations, shapes, test_labels = parse_nap_detector_outputs(
        test_results
    )
    test_activations_tensor = torch.cat(test_activations).cuda()
    test_labels_np = torch.cat(test_labels).numpy()

    test_activations_tensor, _ = prepare_patterns(
        test_activations_tensor, shapes, int(args.layer_id)
    )

    test_distances = monitor.compute_hamming_distance(
        test_activations_tensor, test_labels_np, mean=args.mean
    )
    # test_distances = test_distances[:, int(args.layer_id)]  # last layer only

    test_outputs_uncertainty = append_uncertainty(
        test_bboxes, test_distances, test_labels
    )  # NAP uncertainty is appended to every bbox
