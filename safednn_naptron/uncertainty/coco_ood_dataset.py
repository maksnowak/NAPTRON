...
class CocoOODDataset(CocoDataset):
    ...
    def __init__(self, is_class_agnostic=False, filter_unknown_imgs=False, **kwargs):
    ...
    def get_cats(self):
        ...
        cats = (
            ...
            else [cat for cat in cats if cat["name"] in self.CLASSES]
        )
        return cats
    ...
    def evaluate_ood(
        ...
        for metric in metrics:
            ...
            if metric.startswith("localization"):
                eval_results = self._evaluate_localization(
                    predictions, metric, logger, eval_results
                )
            elif metric == "bbox_known":
                eval_results = self._evaluate_bbox_known(
                    predictions, metric, logger, eval_results, classwise
                )
            else:
                eval_results = self._evaluate_uncertainty(
                    predictions, eval_results, logger, dump_certainties
                )
    ...
    def _evaluate_localization(self, predictions, metric, logger, eval_results):
        ...
        for item in metric_items:
            ...
            val = float(f"{cocoEval.stats[self.coco_metric_names[item]]:.3f}")
    ...
    def _evaluate_bbox_known(
        self, predictions, metric, logger, eval_results, classwise
    ):
        cocoGt = self.coco
        cocoDt = cocoGt.loadRes(predictions)
        iou_type = "bbox"
        ...
        for metric_item in metric_items:
            ...
            val = float(f"{cocoEval.stats[self.coco_metric_names[metric_item]]:.3f}")
            eval_results[key] = val
        ap = cocoEval.stats[:6]
    ...
    def _evaluate_uncertainty(
        self, predictions, eval_results, logger, dump_certainties=False
    ):
        cocoGt = self.coco
        cocoDt = cocoGt.loadRes(predictions)
        iou_type = "bbox"
        ...
        cocoEval = self._eval(
            cocoEval, proposal_nums, iou_thrs, logger, uncertainty=True
        )
        ...
        cocoEval = CocoEvalOOD(
            cocoGt, cocoDt, iou_type, class_agnostic=True, localization_uncertainty=True
        )
    ...
    def _compute_auroc(self, certainties_filtered):
        auroc_results = dict()
        auroc_results["fp_tp_logit"] = auroc(
            certainties_filtered["fp_logit_certainty"],
            certainties_filtered["tp_logit_certainty"],
        )

        auroc_results["ood_tp_logit"] = auroc(
            certainties_filtered["ood_logit_certainty"],
            certainties_filtered["tp_logit_certainty"],
        )
        auroc_results["fp_tp"] = auroc(
            certainties_filtered["fp_certainty"], certainties_filtered["tp_certainty"]
        )
        auroc_results["ood_tp"] = auroc(
            certainties_filtered["ood_certainty"], certainties_filtered["tp_certainty"]
        )
        return auroc_results

    def _compute_fpr95(self, certainties_filtered):
        fpr95_results = dict()
        fpr95_results["fp_tp_logit"] = fpr_at_95_tpr(
            certainties_filtered["fp_logit_certainty"],
            certainties_filtered["tp_logit_certainty"],
        )

        fpr95_results["ood_tp_logit"] = fpr_at_95_tpr(
            certainties_filtered["ood_logit_certainty"],
            certainties_filtered["tp_logit_certainty"],
        )

        fpr95_results["fp_tp"] = fpr_at_95_tpr(
            certainties_filtered["fp_certainty"], certainties_filtered["tp_certainty"]
        )
        fpr95_results["ood_tp"] = fpr_at_95_tpr(
            certainties_filtered["ood_certainty"], certainties_filtered["tp_certainty"]
        )
    ...
    def _auroc_fpr95(self, certainties, gt_counts, logger):
        ...
        for iou in [0.5, 0.75, 0.9]:
            ...
            for i, aRng in enumerate(areaRngLbl):
                certainties_filtered = {
                    k: self._filter_certainty(v, t, i) for k, v in certainties.items()
                }
                ...
                gt_counts_["gt_id_count"] = (
                    gt_counts_["gt_count"] - gt_counts_["gt_ood_count"]
                )
                # self._log_results(certainties_filtered, fpr95_results, auroc_results, gt_counts_, iou,
                #                           aRng, logger)
                eval_results[str(iou)][aRng] = self._insert_results(
                    certainties_filtered, fpr95_results, auroc_results, gt_counts_
                )
        return eval_results

    def _insert_results(
        self, certainties_filtered, fpr95_results, auroc_results, gt_counts_
    ):
        eval_results = OrderedDict()
        ...
        eval_results["miscl"] = int(
            certainties_filtered["tpl_certainty"].shape[0]
            - eval_results["tps"]
            - eval_results["ood"]
        )
    ...
    def _log_results(
        self, certainties, fpr95_results, auroc_results, gt_counts_, iou, area, logger
    ):
        print_log(
            f'IOU: {iou} area: {area} num_fps: {certainties["fp_certainty"].shape} known groundtruths: {gt_counts_["gt_id_count"]}'
            f' unknown groundtruths: {gt_counts_["gt_ood_count"]}'
            f' num_tps: {certainties["tp_certainty"].shape} num_ood: {certainties["ood_certainty"].shape}',
            logger=logger,
        )
        print_log(
            f'False positives vs true positives auroc {auroc_results["fp_tp"]}',
            logger=logger,
        )
        print_log(
            f'False positives vs true positives FPR at 95 TPR {fpr95_results["fp_tp"]}',
            logger=logger,
        )
        print_log(
            f'OOD vs true positives auroc {auroc_results["ood_tp"]}', logger=logger
        )
        print_log(
            f'OOD vs true positives FPR at 95 TPR {fpr95_results["ood_tp"]}',
            logger=logger,
        )
        print_log(
            f'LOGIT False positives vs true positives auroc {auroc_results["fp_tp_logit"]}',
            logger=logger,
        )
        print_log(
            f'LOGIT False positives vs true positives FPR at 95 TPR {fpr95_results["fp_tp_logit"]}',
            logger=logger,
        )
        print_log(
            f'LOGIT OOD vs true positives auroc {auroc_results["ood_tp_logit"]}',
            logger=logger,
        )
        print_log(
            f'LOGIT OOD vs true positives FPR at 95 TPR {fpr95_results["ood_tp_logit"]}',
            logger=logger,
        )
