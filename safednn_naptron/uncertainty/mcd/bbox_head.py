...
class MCDBBoxHead(Shared2FCBBoxHead):

    def __init__(self, dropout_rates=(0.1, 0.0, 0.0), *args, **kwargs):
        super(MCDBBoxHead, self).__init__(*args, **kwargs)
        self.dropouts = [nn.Dropout(d) for d in dropout_rates]
    ...
    def forward(self, x):
        ...
        if self.num_shared_fcs > 0:
            ...
            for i, fc in enumerate(self.shared_fcs):
                ...
                x = self.relu(fc(x))
        # separate branches
        x = self.dropouts[self.num_shared_fcs](x)
        x_cls = x
        x_reg = x
