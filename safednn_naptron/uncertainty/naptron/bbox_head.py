...
class NAPTRONBBoxHead(Shared2FCBBoxHead):
    ...
    def forward(self, x):
        ...
        if self.num_shared_fcs > 0:
            ...
            activations = torch.nn.AdaptiveAvgPool2d(1)(x).flatten(1)
            shapes.append(activations.shape[-1])
            x = x.flatten(1)  # (256, 7, 7) -> 12544

            for fc in self.shared_fcs:
                x = self.relu(fc(x))  # 12544 -> 1024

                activations = torch.cat((activations.cpu(), x.cpu()), dim=1)
