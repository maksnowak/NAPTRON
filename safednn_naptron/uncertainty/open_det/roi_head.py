...
class ODRoiHead(StandardRoIHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ODRoiHead, self).__init__(*args, **kwargs)
        self.curr_iter = 0
    ...
    def _bbox_forward_train(self, x, sampling_results, gt_bboxes, gt_labels, img_metas):
        ...
        ious = torch.cat(
            [
                torch.cat(
                    (
                        p.pos_assigned_ious,
                        torch.zeros(p.neg_inds.shape[0], device=rois.device),
                    )
                )
                for p in sampling_results
            ],
            dim=0,
        )
        gt_labels = torch.cat(
            [
                torch.cat(
                    (
                        p.pos_gt_labels,
                        torch.ones(
                            p.neg_inds.shape[0], device=rois.device, dtype=torch.int64
                        )
                        * self.num_known_classes,
                    )
                )
        ...
        self._dequeue_and_enqueue(
            bbox_results["mlp_feats"],
            gt_labels,
            ious,
            iou_thr=self.ic_loss_queue_iou_thr,
        )
        losses = {
