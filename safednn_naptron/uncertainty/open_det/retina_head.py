...
class ODRetinaHead(RetinaHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(ODRetinaHead, self).__init__(*args, **kwargs)
        self.curr_iter = 0
        ...
        self.queue_label = (
            torch.empty(
                (self.num_known_classes, ic_loss_queue_size),
                device=torch.device("cuda:0" if torch.cuda.is_available() else "cpu"),
            )
            .fill_(-1)
            .long()
        )
        self.queue_ptr = torch.zeros(
    ...
    def _dequeue_and_enqueue(self, feat, gt_classes, ious, iou_thr=0.7):
        ...
        for i in range(self.num_known_classes):
            ...
            self.queue_label[i, ptr : ptr + batch_size] = cls_gt_classes[
                :batch_size
            ].detach()
    ...
    def loss_single(
        ...
        ious = ious.reshape(-1)
        mlp_feats = mlp_feats.permute(0, 2, 3, 1).reshape(-1, self.ic_loss_out_dim)
    ...
    def _get_targets_single(
        ...
        if unmap_outputs:
            ...
            ious = unmap(
                ious, num_total_anchors, inside_flags, fill=0.0
            )  # fill bg label
        ...
        **kwargs
        ...
        if with_nms:
            ...
            det_bboxes = torch.cat(
                (det_bboxes, torch.softmax(det_logits, dim=1)[:, -1].unsqueeze(1)), 1
            )
...
class UPLoss(nn.Module):
    ...
    def forward(self, scores, labels):
        ...
        gt_scores = torch.gather(F.softmax(scores, dim=1), 1, labels[:, None]).squeeze(
            1
        )
        ...
        targets[:, self.num_classes - 2] = gt_scores * (1 - gt_scores).pow(self.alpha)
