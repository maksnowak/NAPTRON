...
class CocoEvalOOD(COCOeval):
    ...
    def __init__(
        ...
        cocoDt=None,
        iouType="segm",
        ignore_ood_gts=False,
        known_category_names=None,
        class_agnostic=False,
        localization_uncertainty=False,
    ):
        """
        ...
        *SA*
        ...
        if not cocoGt is None:
            if known_category_names:
                self.known_category_ids = sorted(
                    cocoGt.getCatIds(catNms=known_category_names)
                )
    ...
    def evaluateImg(self, imgId, catId, aRng, maxDet):
        ...
        ***SAFEDNN MODIFICATION***
        ...
        gt_ood = np.array(
            [g["ood"] if "ood" in g and not g["_ignore"] else 0 for g in gt]
        )
        dtIg = np.zeros((T, D))
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    dtm_ood[tind, dind] = (
                        gt[m]["id"] if "ood" in gt[m] and gt[m]["ood"] else 0
                    )
                    gtm[tind, m] = d["id"]
        ...
        return {
            ...
            "dtUncertaintyScores": [
                d["uncertainty_score"] if "uncertainty_score" in d else 0.0 for d in dt
            ],
        ...
        for k, k0 in enumerate(k_list):
            ...
            for a, a0 in enumerate(a_list):
                ...
                for m, maxDet in enumerate(m_list):
                    ...
                    dtUncertaintyScores = np.concatenate(
                        [e["dtUncertaintyScores"][0:maxDet] for e in E]
                    )
                    ...
                    gtm = np.concatenate(
                        [e["gtMatches"][:, 0:maxDet] for e in E], axis=1
                    )
                    gtm_ood = np.concatenate(
                        [e["gtMatchesOOD"][:, 0:maxDet] for e in E], axis=1
                    )
                    dtm_ood = np.concatenate(
                        [e["dtMatchesOOD"][:, 0:maxDet] for e in E], axis=1
                    )[:, inds]
                    dtIg = np.concatenate(
                        ...
                        for t in range(T):
                            if _pe.useCats == 0 and self.known_category_ids:
                                ...
                                if isinstance(uncertainty_scores[t].get(a), np.ndarray):
                                    uncertainty_scores[t][a] = np.concatenate(
                                        (uncertainty_scores[t].get(a), u_s)
                                    )

                                    bbox_ids[t][a] = np.concatenate(
                                        (bbox_ids[t].get(a), b_s)
                                    )
                                    bbox_img_id[t][a] = np.concatenate(
                                        (bbox_img_id[t].get(a), i_s)
                                    )
                                    bbox_label[t][a] = np.concatenate(
                                        (bbox_label[t].get(a), c_s)
                                    )
                                    logit_scores[t][a] = np.concatenate(
                                        (logit_scores[t].get(a), l_s)
                                    )
                                else:
                                    uncertainty_scores[t][a] = u_s
                                    bbox_ids[t][a] = b_s
    ...
    def summarize(self, uncertainty=False):
        ...
        def _summarize(ap=1, iouThr=None, areaRng="all", maxDets=100):
            ...
            if uncertainty:
                ...
                print(
                    iStr.format(
                        "Mean uncertainty", "", iouStr, areaRng, maxDets, mean_u
                    )
                )
                print(
                    iStr.format(
                        "Mean logit uncertainty", "", iouStr, areaRng, maxDets, mean_l
                    )
                )
                return mean_l
            if ap == 1:
