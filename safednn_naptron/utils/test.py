...
def parse_args():
    ...
    parser.add_argument(
        "--override-batch-size",
        action="store_true",
        help="override progress bar update speed",
    )
    parser.add_argument(
        "--gpu-collect",
        action="store_true",
...
def single_gpu_test(
    ...
    for i, data in enumerate(data_loader):
        ...
        batch_size = 1 if override_batch_size else len(result)
        if show or out_dir:
...
def test(args):
    ...
    output_handler = build_output_handler(cfg)
    ...
    outputs = output_handler(outputs)
    ...
    if rank == 0:
        ...
        if args.eval:
            ...
            try:
                ...
                if not (
                    f"dataset len: {len(outputs)} != {len(dataset)}" in str(e)
                    and "naptron" in args.config
                    and len(outputs) == 4 * len(dataset)
                ):
                    raise
                else:
                    return model, cfg
...
def main():
    args = parse_args()
    print(f"Generating bboxes for config {args.config}")
    test(args)
