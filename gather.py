...
def load_all_results(
    results_dir="results/uncertainty/all_thresholds", pattern="*faster*"
):
    all_files = [
...
def ood_ranking(architecture="retinanet", dataset="voc2coco"):
    ...
    certainties = load_all_results(
        results_dir="./work_dirs/outputs_dump",
        pattern=f"*{architecture}*_{dataset}*_certainties.pkl",
    )
    aurocs = dict()
    fprs = dict()
    aurocs_logit = dict()
    ...
    for a, aRng in enumerate(areaRngLbl):
        for method_filename in certainties:
            ...
            tp_logit_key = (
                "tpl_logit_certainty"
                if "oln" in method_filename
                else "tp_logit_certainty"
            )
            tp_key = "tpl_certainty" if "oln" in method_filename else "tp_certainty"
            tp_label_key = (
                "tpl_bbox_label" if "oln" in method_filename else "tp_bbox_label"
            )
            ...
            for label in range(n_classes):
                ...
                a_l.append(
                    auroc(
                        certainties[method_filename]["ood_certainty"][0][a][mask_ood],
                        certainties[method_filename][tp_key][0][a][mask],
                        reverse_metrics,
                    )
                )
                a_l_l.append(
                    auroc(
                        certainties[method_filename]["ood_logit_certainty"][0][a][
                            mask_ood
                        ],
                        certainties[method_filename][tp_logit_key][0][a][mask],
                        reverse_metrics,
                    )
                )
                f_l.append(
    ...
    for a in aurocs:
        ...
        ranking[a.split("_")[method_index_in_filename]] = (
            s if not np.isclose(s, 0.95) else s_l
        )

    print("FPR@95TPR", ranking)
    sorted_keys = [key for (key, value) in sorted(ranking.items(), key=lambda x: x[1])]
    sorted_vals = [
        value for (key, value) in sorted(ranking.items(), key=lambda x: x[1])
    ]
    ...
    for a in aurocs:
        ...
        ranking[a.split("_")[method_index_in_filename]] = (
            s if not np.isclose(s, 0.5) else s_l
        )
